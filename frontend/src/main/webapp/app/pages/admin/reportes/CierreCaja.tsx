...
import { Container, Row, Col, Card, CardBody, CardTitle, Table, Badge, Button, Input, FormGroup, Label, Alert } from 'reactstrap';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faArrowLeft, faMoneyBillWave, faPrint } from '@fortawesome/free-solid-svg-icons';
...
const CierreCaja = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const ventasDia = resVentas.data.filter(v => dayjs(v.fecha).isSame(fechaSeleccionada, 'day') && !v.anulada);
        ...
        <Row className="g-4 mb-4">
          ...
          <Col md="4">
            <Card className="shadow-sm border-0 h-100 bg-primary text-white" style={{ borderRadius: '15px' }}>
              <CardBody className="p-4">
                <h6 className="fw-bold text-uppercase opacity-75 border-bottom border-white border-opacity-25 pb-2 mb-3 text-white">
                  Efectivo en Caja
                </h6>
                ...
                <div className="d-flex justify-content-between mb-2">
                  <span className="opacity-75" title="No suma al efectivo de la gaveta">
                    + Ventas Tarjeta
                  </span>
                          ...
                          return (
                            <tr key={isVenta ? `v-${op.id}` : `d-${op.id}`} className="small">
                              ...
                              <td className="text-center">
                                {isVenta ? op.usuario?.nombre || 'Admin' : op.venta?.usuario?.nombre || 'Admin'}
                              </td>
                              ...
                              <td className="text-center">
                                <Badge
                                  color={
                                    isVenta
                                      ? op.metodoPago === 'EFECTIVO'
                                        ? 'success'
                                        : 'info'
                                      : op.venta?.metodoPago === 'EFECTIVO' || !op.venta?.metodoPago
                                        ? 'success'
                                        : 'info'
                                  }
                                  pill
                                ...
                                >
                                  {isVenta
                                    ? op.metodoPago === 'TARJETA_STRIPE'
                                      ? 'TARJETA'
                                      : op.metodoPago
                                    : op.venta?.metodoPago === 'TARJETA_STRIPE'
                                      ? 'TARJETA'
                                      : op.venta?.metodoPago || 'EFECTIVO'}
                                </Badge>
                              </td>
          ...
          <Col md="4">
            ...
            <Card className="shadow-sm border-0 border-top border-4 border-success" style={{ borderRadius: '15px' }}>
              <CardBody className="p-3">
                <h6
                  className="fw-bold text-success mb-2 text-center text-uppercase"
                  style={{ fontSize: '0.78rem', letterSpacing: '0.05em' }}
                >
                  Cuadre de Efectivo
                </h6>
                <FormGroup className="mb-2">
                  <Label className="small fw-bold text-muted mb-1 text-center d-block" style={{ fontSize: '0.72rem' }}>
                    MONTO FÍSICO CONTADO
                  </Label>
                  <div className="position-relative">
                    <span
                      className="position-absolute h-100 d-flex align-items-center px-3 text-muted border-end"
                      style={{ top: 0, left: 0, fontSize: '0.82rem' }}
                    >
                      C$
                    </span>
                    <Input
                ...
                {montoFisico !== '' && (
                  <div
                    className={`p-2 rounded-3 text-center mb-2 ${diferencia === 0 ? 'bg-light' : diferencia > 0 ? 'bg-success bg-opacity-10' : 'bg-danger bg-opacity-10'}`}
                  >
                    <div className="fw-bold text-muted text-uppercase mb-1" style={{ fontSize: '0.68rem' }}>
                      Diferencia / Descuadre
                    </div>
                    <div
                      className={`fw-bold ${diferencia === 0 ? 'text-dark' : diferencia > 0 ? 'text-success' : 'text-danger'}`}
                      style={{ fontSize: '1.3rem' }}
                    >
                      {diferencia >= 0 ? '+' : ''} C$ {diferencia.toLocaleString()}
                    </div>
                    <div
                      className="small mt-1 px-2 py-1 rounded-pill d-inline-block fw-bold"
                      style={{ backgroundColor: 'white', fontSize: '0.72rem' }}
                    >
                ...
                <Card className="border-0 bg-light p-2 mb-2" style={{ borderRadius: '10px' }}>
                  <FormGroup className="mb-0">
                    <Label className="small fw-bold text-muted text-uppercase mb-1" style={{ fontSize: '0.72rem' }}>
                      Fondo para Siguiente Caja
                    </Label>
                    <div className="position-relative">
                      <span className="position-absolute h-100 d-flex align-items-center ps-3 text-muted" style={{ fontSize: '0.82rem' }}>
                        C$
                      </span>
                      <Input
                        type="number"
                ...
                <div className="p-2 border-top border-bottom mb-2">
                  <div className="d-flex justify-content-between align-items-center">
                    <span className="fw-bold text-muted text-uppercase" style={{ fontSize: '0.72rem' }}>
                      Monto a Entregar
                    </span>
                    <span className="fw-bold text-primary" style={{ fontSize: '1.1rem' }}>
                      C$ {montoAEntregar.toLocaleString()}
                    </span>
                  </div>
                </div>

                <FormGroup className="mb-2">
                  <Label className="small fw-bold text-muted mb-1" style={{ fontSize: '0.72rem' }}>
                    OBSERVACIONES
                  </Label>
                  <Input
                    type="textarea"
                ...
                <Button
                  ...
                  block
                  size="sm"
                  className="fw-bold mt-1 shadow-sm"
                  style={{ borderRadius: '8px' }}
                  disabled={!montoFisico || saving}
      ...
      <div style={{ display: 'none' }}>
        ...
        >
          <div style={{ textAlign: 'center', marginBottom: '30px', borderBottom: '2px solid black', paddingBottom: '10px' }}>
            ...
            <div style={{ fontSize: '12px', fontWeight: 'bold', color: 'black', marginTop: '10px' }}>
              Emitido: {dayjs().format('DD/MM/YYYY hh:mm A')}
            </div>
          </div>
          ...
          <div style={{ border: '1px solid black', padding: '20px', marginBottom: '30px', fontFamily: 'monospace', fontSize: '13px' }}>
            <div style={{ textAlign: 'center', fontWeight: 'bold', fontSize: '14px', marginBottom: '12px', letterSpacing: '1px' }}>
            ...
            <div
              style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px', color: diferencia >= 0 ? 'green' : 'red' }}
            >
          ...
          <h5 style={{ fontWeight: 'bold', borderBottom: '1px solid black', paddingBottom: '5px', marginTop: '30px' }}>
            ...
            <tbody>
              ...
              {Object.entries(
                ...
                <tr key={vendedor}>
                  ...
                  <td style={{ border: '1px solid black', padding: '10px', textAlign: 'right', fontWeight: 'bold' }}>
