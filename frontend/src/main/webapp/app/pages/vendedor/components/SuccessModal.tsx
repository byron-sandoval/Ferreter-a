            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px', position: 'relative', zIndex: 1 }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: '12.5px' }}>
                  <p style={{ margin: '1px 0' }}><strong>Cédula/RUC:</strong> {ventaExitosa?.cliente?.cedula}</p>
                  <p style={{ margin: '1px 0' }}><strong>Dirección:</strong> {ventaExitosa?.cliente?.direccion || 'Ciudad'}</p>
                  <p style={{ margin: '1px 0' }}><strong>Teléfono:</strong> {ventaExitosa?.cliente?.telefono || 'N/A'}</p>
                </div>
              </div>
              <div style={{ backgroundColor: '#fff', padding: '10px', borderRadius: '10px', border: '2px solid #fd7e14', minWidth: '220px', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
                ...
                <tbody>
                  {carrito.map((item, i) => (
                    <tr key={i} style={{ borderBottom: '1.2px solid #ccc' }}>
                      ...
                      <td style={{ padding: '8px 15px', textAlign: 'right', fontSize: '12px', fontWeight: 'bold' }}>C$ {item.subtotal.toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div style={{ display: 'flex', width: '100%', border: '1.5px solid #ccc', marginTop: '-1.5px', position: 'relative', zIndex: 1, backgroundColor: '#fff' }}>
              ...
              <div style={{ width: '40%' }}>
                <div style={{ display: 'flex', borderBottom: '1.2px solid #ccc' }}>
                  ...
                  <div style={{ width: '50%', padding: '8px', textAlign: 'right', fontSize: '11.5px' }}>C$ {ventaExitosa?.subtotal?.toFixed(2)}</div>
                </div>
                <div style={{ display: 'flex', borderBottom: '1.2px solid #ccc' }}>
                  ...
                  <div style={{ width: '50%', padding: '8px', textAlign: 'right', fontSize: '11.5px', color: '#d9534f' }}>- C$ {(ventaExitosa?.descuento || 0).toFixed(2)}</div>
                </div>
                <div style={{ display: 'flex', borderBottom: '1.2px solid #ccc' }}>
                  ...
                  <div style={{ width: '50%', padding: '8px', textAlign: 'right', fontSize: '11.5px' }}>C$ {ventaExitosa?.iva?.toFixed(2)}</div>
                </div>
                <div style={{ display: 'flex', backgroundColor: '#fef3e7' }}>
                  ...
                  <div style={{ width: '50%', padding: '10px 8px', textAlign: 'right', fontWeight: 'bold', fontSize: '16px', color: '#fd7e14' }}>C$ {ventaExitosa?.total?.toFixed(2)}</div>
                </div>
